			...
			<ul class="pagination justify-content-center">
				<c:if test="${currentPage != 1}">
					<li class="page-item"><a class="page-link"
						href="${root}/house/aptFind?aptName=${aptName}&page=1">&lt;&lt;</a></li>
				</c:if>
				<c:if test="${currentPage != 1}">
					<li class="page-item"><a class="page-link"
						href="${root}/house/aptFind?aptName=${aptName}&page=${currentPage - 1}">&lt;</a></li>
				</c:if>
				...
				<c:forEach begin="${((currentPage-1-((currentPage-1)%5))/5)*5+1}" end="${((currentPage-1-((currentPage-1)%5))/5)*5+5 < noOfPages ? ((currentPage-1-((currentPage-1)%5))/5)*5+5 : noOfPages}" var="i">
					<c:choose>
						<c:when test="${currentPage eq i}">
							<li class="page-item active"><a class="page-link"
								href="${root}/house/aptFind?aptName=${aptName}&page=${i}">${i}</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link"
								href="${root}/house/aptFind?aptName=${aptName}&page=${i}">${i}</a></li>
				...
				<c:if test="${currentPage != noOfPages}">
					<li class="page-item"><a class="page-link"
						href="${root}/house/aptFind?aptName=${aptName}&page=${noOfPages}">&gt;&gt;</a></li>
				</c:if>
			</ul>
		</div>
	</section>
